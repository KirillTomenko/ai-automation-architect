...
Таксономия и каталог — редактируемые YAML-конфиги, промпты их подставляют, а не дублируют. Стадии 1–3 работают на низкой температуре (извлечение и классификация должны быть воспроизводимыми); вход и выход каждой стадии логируются отдельно — пайплайн отлаживается по стадиям, а не «всё сразу».
...
Фронтенд: **http://localhost:8000/** — форма ввода и выпадающий список из 4 готовых кейсов (рендерятся без вызова LLM). «Построить Blueprint» — живой прогон через 4 стадии, обычно 20–60 секунд. Программный доступ: `POST /blueprint` с телом `{"description": "..."}`.
